...
#include <string>
using namespace std;
wstring conversor_ot( vector<wstring>a){
	wstring b =L"";
...
inicio:		
		//limite de arquvos music=20;
		bool work = false;
		namespace fs = experimental::filesystem;
		//"C:\Users\Home\Desktop\projects\C++SFML\ppm_v2\musicteste\Release"
		//set_by_user
		string set_path;
		printf("%s", "_DIR:");
		cin >>set_path;
		...
		while (true){
			...
			if (fs::exists(p)) { cout << set_path << "===>" << "Setado como PATH" << endl; 
		...
		vector <wstring> music;
		wstring string_dr = L"DIR:";
		wstring path_str=p;
		...
		wstring out=L"Arquivos .ogg Não Encontrados no PATH";
			...
			//_vol;
				...
				if (sf::Keyboard::isKeyPressed(sf::Keyboard::Enter)&&work) {
					if (cm != 1) {
						wstring dir = L"\\";
						wstring w = p;
						m.openFromFile(wtos(w)+wtos(dir)+wtos(music[cm-1]));
						m.play();
						wstring track_l = L"TRACK:";
